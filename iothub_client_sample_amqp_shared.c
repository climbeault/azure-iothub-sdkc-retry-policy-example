...
static void ConnectionStatusCallback(IOTHUB_CLIENT_CONNECTION_STATUS result, IOTHUB_CLIENT_CONNECTION_STATUS_REASON reason, void* connectionStatusContextCallback)
{
	(void)connectionStatusContextCallback;
	(void)printf("Connection status with result = %s and reason = %s\r\n", ENUM_TO_STRING(IOTHUB_CLIENT_CONNECTION_STATUS, result), ENUM_TO_STRING(IOTHUB_CLIENT_CONNECTION_STATUS_REASON, reason));
}
...
void iothub_client_sample_amqp_shared_hl_run(void)
{
    ...
    else
    {
        ...
        {
			IoTHubClient_SetRetryPolicy(iotHubClientHandle1, IOTHUB_CLIENT_RETRY_NONE, 0);
			IoTHubClient_SetConnectionStatusCallback(iotHubClientHandle1, ConnectionStatusCallback, NULL);
            ...
            if (create_events(messages_device1, client_config1.deviceId) != 0)
            {
